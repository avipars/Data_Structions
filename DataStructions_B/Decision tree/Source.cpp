...
int main()
{
	...
	do
	{
		...
		try
		{
			switch (ch)
			{
			case 'n':cout << "enter the main question (with no space) \n"; cin >> val; t.addRoot(val); break;
			case 's':cout << "enter the question (with no space) \n"; cin >> father;
			...
			case 'd':cout << "enter value of subtree to delete (with no space) \n"; cin >> val; t.deleteSubTree(val);  break;
			case 'p':t.printAllTree();  break;
			case 'w':cout << "enter a value (with no space) \n"; cin >> val; t.searchAndPrint(val); cout << endl;  break;
